/* =========================================
   BATTLE UI CONTROLLER - POLISHED (CINEMATIC V3 - TON EDITION)
   ========================================= */
...
const showLootToast = (message) => {
    const exist = document.getElementById('sys-loot-toast'); 
    ...
    toast.style.cssText = `
        ...
        background: rgba(13,17,23,0.95); border: 1px solid #0098EA; color: #e6edf3; 
        padding: 15px 25px; border-radius: 12px; font-weight: bold; font-size: 12px; 
        z-index: 99999; box-shadow: 0 10px 30px rgba(0,152,234,0.2), inset 0 0 15px rgba(0,152,234,0.05); 
        text-align: center; line-height: 1.6; backdrop-filter: blur(8px); 
        transition: all 0.4s cubic-bezier(0.175, 0.885, 0.32, 1.275); 
        opacity: 0; margin-top: -20px; width: 85%; max-width: 320px;
    `;
    
    toast.innerHTML = `
        <div style="margin-bottom:8px;">
            <img src="source/icon/sub/crate.png" style="width:24px; height:24px; filter:drop-shadow(0 0 5px #0098EA);">
        </div>
        ${message}
    `;
...
// === SISTEM REGEN STAMINA & HP (SETIAP 5 MENIT) ===
setInterval(() => {
    ...
    if (typeof SHIPS !== 'undefined' && SHIPS[shipClass]) {
        maxHp = SHIPS[shipClass].hp;
    }
    ...
    const REGEN_MS = 5 * 60 * 1000; // 5 menit

    // Jika Stamina ATAU HP belum penuh
    ...
    if (modified) {
        ...
        // Jika tidak ada point bertambah, update timer visual saja
...
export const handleAttack = async (monster, event) => {
    ...
    if (curStamina < 1) {
        return showStaminaWarning("WARNING", "Stamina Depleted! Wait for recovery or refill in Shop.");
    }
    ...
    if (event && event.target) {
        ...
        event.target.innerText = "LOCKING ON..."; 
    ...
    if (initialState.equipped) {
        Object.values(initialState.equipped).forEach(item => {
            if (item && item.stats) {
                totalAtk += Number(item.stats.atk) || 0; 
                totalCrit += Number(item.stats.crit) || 0; 
                totalDef += Number(item.stats.def) || 0;
                totalHpMax += Number(item.stats.hp) || 0; 
                totalSpeed += Number(item.stats.speed) || 0; 
            }
        });
    }
    ...
    showBattleAnimationPopup(playerImage, monster.image, result, () => {
        ...
        if (up.currentHp <= 0) {
            ...
            up.currentHp = Math.floor(realStats.hp * 0.1); 
        ...
        if (result.isKilled || result.goldGained > 0) {
            let lootMessage = result.isKilled 
                ? `<span style="color:var(--gold); font-size:14px; text-transform:uppercase; text-shadow:0 0 5px var(--gold);">VICTORY!</span><br>` 
                : `<span style="color:#0098EA; font-size:14px; text-transform:uppercase; text-shadow:0 0 5px #0098EA;">HIT SUCCESS!</span><br>`;
            ...
            // Cek Level up menggunakan logika battle logic
            const lvlResult = checkLevelUp(up, result.xpGained);
            up.xp = lvlResult.xp; 
            up.level = lvlResult.level; 
            up.maxXp = lvlResult.maxXp; 
            up.maxStamina = lvlResult.maxStamina;

            // Jika level up berhasil (fungsi checkLevelUp diharapkan melempar boolean leveledUp)
            if (lvlResult.leveledUp) {
                // Gunakan optional chaining untuk menghindari error bila fungsi global tidak ada
                window.showLevelUpEffect?.(); 
                
                lootMessage += `<br><span style="color:#00eaff; font-weight:900; font-size:14px; letter-spacing:1px; text-shadow:0 0 5px #00eaff;"><img src="source/icon/sub/vip.png" style="width:16px; vertical-align:-3px; margin-right:6px;">LEVEL UP! (Lv.${up.level})</span>`;
            }
            ...
            if (result.lootOre > 0) { 
                up.iron_ore = (up.iron_ore || 0) + result.lootOre; 
                lootMessage += `<br><span style="color:#8b949e;"><img src="source/item/ore.png" style="width:14px; vertical-align:-2px; margin-right:6px;">+${result.lootOre} ORE</span>`; 
            }
            if (result.lootEnergy > 0) { 
                up.dark_energy = (up.dark_energy || 0) + result.lootEnergy; 
                lootMessage += `<br><span style="color:#9b59b6;"><img src="source/item/energy.png" style="width:14px; vertical-align:-2px; margin-right:6px;">+${result.lootEnergy} ENERGY</span>`; 
            }
        ...
        const hpText = document.getElementById(`hp-text-${monster.id}`); 
        const hpBar = document.getElementById(`hp-bar-${monster.id}`);
...
async function showBattleAnimationPopup(playerImgSrc, monsterImgSrc, result, onComplete) {
        ...
        <div class="cinematic-box">
            ...
            <div id="anim-stage" class="cinematic-stage" style="position: relative; height: 150px; display: flex; align-items: center; justify-content: space-between;">
                ...
                <div id="damage-center-zone" style="position: absolute; left: 50%; top: 50%; transform: translate(-50%, -50%); width: 100px; height: 100px; pointer-events: none; z-index: 10;">
                    <div id="dmg-player-text" style="position: absolute; top: 20%; left: 50%; transform: translateX(-50%); opacity: 0; font-weight: 900; font-size: 24px; color: #0098EA; text-shadow: 0 0 10px #000; transition: 0.2s;"></div>
    ...
    logEl.innerText = "Locking onto Target..."; 
    logEl.style.color = "#00eaff";
    await sleep(400);

    logEl.innerText = "Firing Main Laser!"; 
    logEl.style.color = "#0098EA";
    playerEl.style.transform = 'scaleX(-1) translateX(-15px)'; 
    playSFX('laser'); 
    
    const pLaser = document.createElement('div');
    pLaser.style.cssText = 'position:absolute; top:50%; left:20%; height:3px; background:#0098EA; box-shadow:0 0 12px #0098EA; border-radius:2px; z-index:1; transform:translateY(-50%);';
    ...
    if (!result.isKilled && result.monsterDamage > 0) {
        logEl.innerText = "Warning! Enemy Counter-Attack!"; 
        ...
        logEl.innerText = "Target Destroyed!"; 
        logEl.style.color = "#ffca28";
        playSFX('monsterDie'); 
...
export const updateTopBar = () => {
    ...
    if (lvlBadge) lvlBadge.innerText = `LV. ${state.profile.level || 1}`;
...
function showStatusPopup(title, message, iconPath, color) {
    ...
    box.innerHTML = `
        <div class="battle-icon-container">
            <img src="${iconPath}" class="modal-icon-md">
        </div>
        <h3 style="color:${color}; margin-bottom:10px;">${title}</h3>
        <p class="battle-text">${message}</p>
        <button id="btn-status-ok" class="btn-battle-modal mt-20 text-black" style="background:${color};">CONTINUE</button>
    `;
    
    overlay.appendChild(box); 
    document.body.appendChild(overlay); 
    
    document.getElementById('btn-status-ok').onclick = () => overlay.remove();
}

function showStaminaWarning(title, message) {
    const exist = document.getElementById('stamina-popup'); 
    if (exist) exist.remove();
    
    const overlay = document.createElement('div'); 
    overlay.id = 'stamina-popup'; 
    overlay.className = 'battle-overlay';
    
    const box = document.createElement('div'); 
    box.className = 'battle-box battle-box-stamina';
    
    box.innerHTML = `
        <div class="battle-icon-container">
            <img src="source/icon/stamina.png" class="battle-icon-stamina" onerror="this.src='source/icon/sub/danger.png'">
        </div>
        <h3 class="battle-title-stamina">${title}</h3>
        <p class="battle-text">${message}</p>
        <button id="btn-stamina-ok" class="btn-battle-modal btn-stamina">UNDERSTOOD</button>
    `;
    
    overlay.appendChild(box); 
    document.body.appendChild(overlay); 
    
    document.getElementById('btn-stamina-ok').onclick = () => overlay.remove();
}
